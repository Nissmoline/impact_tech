  ...
  return (
    <nav 
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled || isMobileMenuOpen ? 'bg-slate-950/80 backdrop-blur-md py-4 border-b border-white/5' : 'bg-transparent py-6'
      }`}
    >
      <div className="container mx-auto px-6 flex justify-between items-center relative">
        <div className="flex items-center gap-2 text-white font-display font-bold text-xl tracking-tight z-50">
          <Hexagon className="text-cyan-400 fill-cyan-400/20" size={32} strokeWidth={1.5} />
          VERTEX
        </div>
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <button className="px-5 py-2 bg-white/10 hover:bg-white/20 text-white rounded-full text-sm font-medium transition-colors border border-white/5">
            Contact
          </button>
        </div>

        {/* Mobile Toggle Button (3D Flip) */}
        <div className="md:hidden perspective-1000 z-50">
            <motion.button
                onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
                className="relative w-12 h-12 transform-style-3d focus:outline-none"
                animate={{ rotateY: isMobileMenuOpen ? 180 : 0 }}
                transition={{ duration: 0.6, type: "spring", stiffness: 260, damping: 20 }}
            >
                {/* Front Side (Menu) */}
                <div className="absolute inset-0 flex items-center justify-center bg-slate-800/80 border border-white/10 rounded-xl backdrop-blur-md backface-hidden shadow-lg group">
                    <Menu className="text-white group-hover:text-cyan-400 transition-colors" size={24} />
                </div>

                {/* Back Side (Close) */}
                <div 
                    className="absolute inset-0 flex items-center justify-center bg-cyan-900/80 border border-cyan-500/30 rounded-xl backdrop-blur-md backface-hidden shadow-lg shadow-cyan-500/20"
