...
const TiltCard: React.FC<TiltCardProps> = ({ children, className = "", rotationFactor = 15 }) => {
  ...
  const rotateX = useTransform(mouseYSpring, [-0.5, 0.5], [`${rotationFactor}deg`, `-${rotationFactor}deg`]);
  const rotateY = useTransform(mouseXSpring, [-0.5, 0.5], [`-${rotationFactor}deg`, `${rotationFactor}deg`]);
